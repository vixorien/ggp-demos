...
#undef LoadTexture
	...
	// Render target setup
	{
		...
		sdfPS = std::make_shared<SimplePixelShader>(Graphics::Device, Graphics::Context, FixPath(L"SDF_PS.cso").c_str());
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	XMFLOAT4X4 v = camera->GetView();
	XMFLOAT4X4 p = camera->GetProjection();
	XMFLOAT4X4 invVP;
	
	XMMATRIX invVPmat = XMMatrixInverse(0, XMMatrixMultiply(XMLoadFloat4x4(&v), XMLoadFloat4x4(&p)));
	XMStoreFloat4x4(&invVP, invVPmat);

	sdfPS->SetMatrix4x4("inverseViewProjection", invVP);
