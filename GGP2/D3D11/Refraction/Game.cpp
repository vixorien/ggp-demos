...
#undef LoadTexture
	...
	// === Refractive ===
	std::shared_ptr<Material> refractiveMat = std::make_shared<Material>("Refractive", refractionPS, vertexShader, XMFLOAT3(1, 1, 1), XMFLOAT2(2, 2), XMFLOAT2(1,1), true);
	refractiveMat->AddSampler("BasicSampler", sampler);
	refractiveMat->AddSampler("ClampSampler", clampSampler);
	refractiveMat->AddTextureSRV("NormalMap", bronzeN);

	// Add materials to list
	materials.insert(materials.end(), { cobbleMat2x, cobbleMat4x, floorMat, paintMat, scratchedMat, bronzeMat, roughMat, woodMat, refractiveMat });
	...
	for (int i = 0; i < 32; i++)
	{
		...
		case 7: whichMat = refractiveMat; break;
		}
	...
	std::shared_ptr<GameEntity> paintSphere = std::make_shared<GameEntity>(sphereMesh, refractiveMat);
	...
	std::shared_ptr<GameEntity> bronzeSphere = std::make_shared<GameEntity>(sphereMesh, refractiveMat);
...
void Game::ResizeAllRenderTargets()
{
	...
	// Save the new SRV
	demoOptions.ColorSRV = colorSRV;
	demoOptions.SilhouetteSRV = silhouetteSRV;
}
...
void Game::Update(float deltaTime, float totalTime)
{
	...
	BuildUI(camera, meshes, *currentScene, materials, lights, lightOptions, demoOptions);
	...
	// Spin refractive objeects
	for (auto& e : refractionList)
	{
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	// Change the render target to render directly into our color texture
	Graphics::Context->OMSetRenderTargets(1, colorRTV.GetAddressOf(), Graphics::DepthBufferDSV.Get());

	// Reset refraction list for this frame
	...
	{
		// Skip refraction entities for later
		if (e->GetMaterial()->GetRefractive())
		{
			refractionList.push_back(e);
			continue;
		}
