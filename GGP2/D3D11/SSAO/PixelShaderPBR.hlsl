...
PixelOutput main(VertexToPixel input)
{
	...
	// -- Diffuse IBL --
	float3 indirectDiffuse = IrradianceIBLMap.SampleLevel(BasicSampler, input.normal, 0).rgb;
	...
	float2 indirectBRDF = BrdfLookUpMap.Sample(ClampSampler, float2(NdotV, roughness)).rg;
	
	// -- Specular IBL --
	float3 indSpecFresnel = specColor * indirectBRDF.x + indirectBRDF.y;
	float3 indirectSpecular = SpecularIBLMap.SampleLevel(BasicSampler, viewRefl, roughness * (specularIBLTotalMipLevels - 1.0)).rgb * indSpecFresnel;
	
	// -- Total Indirect --
	float3 fullIndirect = (indirectDiffuse * surfaceColor.rgb * saturate(1.0f - metal)) + indirectSpecular;

	// No gamma correction yet!
	PixelOutput output;
	output.colorDirect = float4(totalLight, 1);
	output.colorAmbient = float4(fullIndirect, 1);
