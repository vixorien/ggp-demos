...
void Game::ResizeAllRenderTargets()
{
	...
	ResizeRenderTarget(metalRoughRTV, metalRoughSRV, DXGI_FORMAT_R8G8B8A8_UNORM);

	// Save the new SRV
	demoOptions.ColorSRV = colorSRV;
	demoOptions.NormalsSRV = normalsSRV;
	demoOptions.DepthSRV = depthSRV;
	demoOptions.MetalRoughSRV = metalRoughSRV;
}
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	// - At the beginning of Game::Draw() before drawing *anything*
	{
		...
		Graphics::Context->ClearRenderTargetView(metalRoughRTV.Get(), color);
	}
