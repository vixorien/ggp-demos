...
void Game::Initialize()
{
	...
	camera = std::make_shared<FPSCamera>(
		XMFLOAT3(5.0f, 0.0f, -25.0f),	// Position
		5.0f,					// Move speed
		0.002f,					// Look speed
...
#undef LoadTexture
	...
	std::shared_ptr<SimpleComputeShader> emitCS = std::make_shared<SimpleComputeShader>(Graphics::Device, Graphics::Context, FixPath(L"ParticleEmitCS.cso").c_str());
	std::shared_ptr<SimpleComputeShader> updateCS = std::make_shared<SimpleComputeShader>(Graphics::Device, Graphics::Context, FixPath(L"ParticleUpdateCS.cso").c_str());
	std::shared_ptr<SimpleComputeShader> deadListInitCS = std::make_shared<SimpleComputeShader>(Graphics::Device, Graphics::Context, FixPath(L"ParticleDeadListInitCS.cso").c_str());
	std::shared_ptr<SimpleComputeShader> copyDrawCountCS = std::make_shared<SimpleComputeShader>(Graphics::Device, Graphics::Context, FixPath(L"ParticleCopyDrawCountCS.cso").c_str());

	std::shared_ptr<SimpleComputeShader> flowEmitCS = std::make_shared<SimpleComputeShader>(Graphics::Device, Graphics::Context, FixPath(L"ParticleFlowEmitCS.cso").c_str());
	std::shared_ptr<SimpleComputeShader> flowUpdateCS = std::make_shared<SimpleComputeShader>(Graphics::Device, Graphics::Context, FixPath(L"ParticleFlowUpdateCS.cso").c_str());
	...
	emitters.push_back(std::make_shared<Emitter>(
		...
		XMFLOAT3(0, 0, 0),		// Start velocity
		XMFLOAT3(0, 0, 0),		// Velocity randomness range
		XMFLOAT3(20, 0, 0),		// Emitter position
		XMFLOAT3(5, 5, 5),		// Position randomness range
		XMFLOAT2(-10, 10),		// Random rotation - startMin, startMax
		XMFLOAT2(-10, 10),		// Random rotation - endMin, endMax
		XMFLOAT3(0, 0, 0),		// Constant acceleration
		starParticle));
