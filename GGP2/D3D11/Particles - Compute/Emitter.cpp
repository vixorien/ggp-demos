		...
		timeSinceLastEmit(0)
{
	...
	// the number of particles in this emitter)
	{
		// DRAW ARGUMENTS ================
		{
			...
			Graphics::Device->CreateBuffer(&argsDesc, 0, drawArgsBuffer.GetAddressOf());
			...
			Graphics::Device->CreateUnorderedAccessView(drawArgsBuffer.Get(), &argsUAVDesc, drawArgsUAV.GetAddressOf());
		}
		...
		// DEAD LIST COUNTER BUFFER ==================
		{
			...
			Graphics::Device->CreateBuffer(&deadCounterDesc, 0, deadListCounterBuffer.GetAddressOf());
		}
	}
...
void Emitter::CreateGPUResources()
{
	...
	// PARTICLE POOL ============
	{
		...
		Graphics::Device->CreateBuffer(&poolDesc, 0, particlePoolBuffer.GetAddressOf());
		...
		Graphics::Device->CreateUnorderedAccessView(particlePoolBuffer.Get(), &poolUAVDesc, particlePoolUAV.GetAddressOf());
		...
		Graphics::Device->CreateShaderResourceView(particlePoolBuffer.Get(), &poolSRVDesc, particlePoolSRV.GetAddressOf());
	}
	...
	// DEAD LIST ===================
	{
		...
		Graphics::Device->CreateBuffer(&deadDesc, 0, deadListBuffer.GetAddressOf());
		...
		Graphics::Device->CreateUnorderedAccessView(deadListBuffer.Get(), &deadUAVDesc, particleDeadUAV.GetAddressOf());
	}
	...
	// Draw List
	{
		...
		Graphics::Device->CreateBuffer(&drawDesc, 0, drawListBuffer.GetAddressOf());
		...
		Graphics::Device->CreateUnorderedAccessView(drawListBuffer.Get(), &drawUAVDesc, particleDrawUAV.GetAddressOf());
		...
		Graphics::Device->CreateShaderResourceView(drawListBuffer.Get(), &drawSRVDesc, particleDrawSRV.GetAddressOf());
	}
	...
	// Populate dead list
	{
		...
		Graphics::Context->CopyStructureCount(deadListCounterBuffer.Get(), 0, particleDeadUAV.Get());
	}
}
...
void Emitter::Update(float dt, float currentTime)
{
	...
	if (timeSinceLastEmit > secondsPerParticle)
	{
		...
		Graphics::Context->CSSetConstantBuffers(1, 1, deadListCounterBuffer.GetAddressOf()); // Manually setting a whole cbuffer here
...
void Emitter::Draw(std::shared_ptr<Camera> camera, float currentTime, bool debugWireframe)
{
	...
	// PREPARE DRAW DATA ===============
	{
		...
		// Reset here too (may be unnecessary)
		Graphics::Context->CSSetUnorderedAccessViews(0, 8, none, 0);
	...
	Graphics::Context->VSSetShaderResources(0, 1, particlePoolSRV.GetAddressOf());
	Graphics::Context->VSSetShaderResources(1, 1, particleDrawSRV.GetAddressOf());
