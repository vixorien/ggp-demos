...
void Game::Initialize()
{
	...
	// Create 5 RW textures - albedo, normals, roughness, metalness and noise
	RWTextures.push_back(CreateRWTexture("Noise", 512, 512));
	RWTextures.push_back(CreateRWTexture("Albedo", 512, 512));
	RWTextures.push_back(CreateRWTexture("Normal Map", 512, 512));
	RWTextures.push_back(CreateRWTexture("Roughness", 512, 512));
	RWTextures.push_back(CreateRWTexture("Metalness", 512, 512));
...
void Game::CreateGeometry()
{
	...
	std::shared_ptr<Material> computeMat = std::make_shared<Material>(pipelineState, XMFLOAT3(1, 1, 1));
	computeMat->SetAlbedoIndex(RWTextures[1].DescriptorHeapIndex);
	computeMat->SetNormalMapIndex(RWTextures[2].DescriptorHeapIndex);
	computeMat->SetRoughnessIndex(RWTextures[3].DescriptorHeapIndex);
	computeMat->SetMetalnessIndex(RWTextures[4].DescriptorHeapIndex);
	...
	// Create entities
	std::shared_ptr<GameEntity> entityCube = std::make_shared<GameEntity>(cube, computeMat);
...
RWTextureDetails Game::CreateRWTexture(std::string name, unsigned int width, unsigned int height)
{
	// Create the texture
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> rwTexture;
	Graphics::Device->CreateCommittedResource(
		...
		0,
		IID_PPV_ARGS(rwTexture.GetAddressOf()));

	// Set up details
	RWTextureDetails details{};
	details.Resource = rwTexture;

	// Reserve a slot
	D3D12_CPU_DESCRIPTOR_HANDLE cpu;
	Graphics::ReserveDescriptorHeapSlot(&cpu, &details.GPUHandle);
	details.DescriptorHeapIndex = Graphics::GetDescriptorIndex(details.GPUHandle);
	...
	Graphics::Device->CreateUnorderedAccessView(rwTexture.Get(), 0, &uavDesc, cpu);
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	// Prepare compute pipeline and run shaders
	{
		...
		computeData.noiseTextureIndex = RWTextures[0].DescriptorHeapIndex;
		computeData.albedoTextureIndex = RWTextures[1].DescriptorHeapIndex;
		computeData.normalTextureIndex = RWTextures[2].DescriptorHeapIndex;
		computeData.roughTextureIndex = RWTextures[3].DescriptorHeapIndex;
		computeData.metalTextureIndex = RWTextures[4].DescriptorHeapIndex;
		computeData.time = totalTime;
...
void Game::BuildUI()
{
	...
	ImGui::Begin("Inspector");
	{
		...
		// Draw the texture generated by compute
		ImGui::Spacing();
		if (ImGui::TreeNode("Textures - Full Size"))
		{
			for (int i = 0; i < RWTextures.size(); i++)
			{
				ImGui::Spacing();
				ImGui::Text(RWTextures[i].name.c_str());
				ImGui::Image(RWTextures[i].GPUHandle.ptr, ImVec2(512, 512));
			}
		...
		for (int i = 0; i < RWTextures.size(); i++)
		{
			ImGui::Image(RWTextures[i].GPUHandle.ptr, ImVec2(128, 128));
			ImGui::SameLine();
		}
	}
	ImGui::End();
