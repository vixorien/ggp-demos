...
void Game::Initialize()
{
	...
	// Set up initial light ray data
	skyDrawData.falloffExponent = 128.0f;
	skyDrawData.sunColor = XMFLOAT3(1, 1, 1);
	skyDrawData.useSkyboxColor = false;
	skyDrawData.sunDirection = XMFLOAT3(0, 0, 1);

	lightRayData.decay = 0.98f;
	lightRayData.density = 1.0f;
	lightRayData.exposure = 0.2f;
	lightRayData.numSamples = 128;
	lightRayData.weight = 0.2f;
}
...
void Game::CreateRootSigAndPipelineState()
{
	...
	Microsoft::WRL::ComPtr<ID3DBlob> lightRayPS;
	...
	// Load shaders
	{
		...
		D3DReadFileToBlob(FixPath(L"LightRayPS.cso").c_str(), lightRayPS.GetAddressOf());
	}
	...
	// Root Signatures
	{
		...
		// Light ray root sig ====
		rootParams->Constants.Num32BitValues = sizeof(LightRayData) / sizeof(unsigned int);
		...
		Graphics::Device->CreateRootSignature(
			...
			serializedRootSig->GetBufferPointer(),
			serializedRootSig->GetBufferSize(),
			IID_PPV_ARGS(lightRayRootSig.GetAddressOf()));
	}
	...
	// Pipeline state
	{
		...
		psoDesc.pRootSignature = lightRayRootSig.Get();

		psoDesc.PS.BytecodeLength = lightRayPS->GetBufferSize();
		psoDesc.PS.pShaderBytecode = lightRayPS->GetBufferPointer();
		...
		Graphics::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(lightRayPSO.GetAddressOf()));
	...
	RenderTargets[SunOcclusionRT] = Graphics::CreateTexture(Window::Width(), Window::Height(), 1, 1, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET);
	...
	Graphics::Device->CreateRenderTargetView(RenderTargets[SunOcclusionRT].Texture.Get(), &rtvDesc, RenderTargets[SunOcclusionRT].RTV);
	...
	Graphics::Device->CreateShaderResourceView(RenderTargets[SunOcclusionRT].Texture.Get(), &srv, RenderTargets[SunOcclusionRT].SRV.CPUHandle);
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	// Rendering here!
	{
		...
		Graphics::CommandList->OMSetRenderTargets(
			MaxRenderTargets,             // Set all at once
			&RenderTargets[ColorRT].RTV,  // Address of first
			true,                         // True means all are next to each other
	...
	{
		Graphics::CommandList->SetPipelineState(lightRayPSO.Get());
		Graphics::CommandList->SetGraphicsRootSignature(lightRayRootSig.Get());

		// Determine light's current screen space position
		XMFLOAT4X4 view = camera->GetView();
		XMFLOAT4X4 proj = camera->GetProjection();
		XMVECTOR lightPosWorld = XMLoadFloat3(&skyDrawData.sunDirection);
		...
		XMStoreFloat2(&lightRayData.lightPosScreenSpace, lightPosScreenVec);

		lightRayData.SceneColorIndex = RenderTargets[ColorRT].SRV.GPUDescriptorIndex;
		lightRayData.SunOcclusionIndex = RenderTargets[SunOcclusionRT].SRV.GPUDescriptorIndex;


		// Set data
		Graphics::CommandList->SetGraphicsRoot32BitConstants(
			0, 
			sizeof(LightRayData) / sizeof(unsigned int), 
			&lightRayData,
			0);
...
void Game::BuildUI()
{
	...
	ImGui::Begin("Inspector");
	{
		...
		// === Light Rays ===
		if (ImGui::TreeNode("Volumetric Light Rays"))
		{
			ImGui::Text("Sun Details");
			ImGui::SliderFloat3("Direction to Sun", &skyDrawData.sunDirection.x, -1.0f, 1.0f);
			ImGui::SliderFloat("Sun Falloff Exponent", &skyDrawData.falloffExponent, 1.0f, 512.0f);
			ImGui::Checkbox("Use Skybox Color For Sun", reinterpret_cast<bool*>(&skyDrawData.useSkyboxColor));
			if (!skyDrawData.useSkyboxColor)
				ImGui::ColorEdit3("Sun Color", &skyDrawData.sunColor.x);
			ImGui::Spacing();

			ImGui::Text("Light Rays");
			ImGui::SliderInt("Num Samples", &lightRayData.numSamples, 2, 256);
			ImGui::SliderFloat("Volume Density", &lightRayData.density, 0.01f, 1.0f);
			ImGui::SliderFloat("Sample Weight", &lightRayData.weight, 0.0f, 1.0f);
			ImGui::SliderFloat("Light Decay", &lightRayData.decay, 0.0f, 1.0f);
			ImGui::SliderFloat("Overall Exposure", &lightRayData.exposure, 0.0f, 2.0f);
