...
struct PS_Output
{
	...
	float4 SunOccluders  : SV_TARGET1;
};
...
PS_Output main(VertexToPixel input)
{
	...
	PS_Output output;
	output.Color = float4(totalLight, 1); // No gamma correction yet!
	output.SunOccluders = float4(0, 0, 0, 1); // Black since this object occludes the sky
	return output;
}