...
VertexToPixel main(VertexShaderInput input)
{
	...
	output.normal = normalize(mul((float3x3)cb.worldInverseTranspose, input.normal));
	output.tangent = normalize(mul((float3x3)cb.world, input.tangent));

	// Calc vertex world pos
	output.worldPos = mul(cb.world, float4(input.localPosition, 1.0f)).xyz;
