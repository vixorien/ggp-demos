...
void Game::Initialize()
{
	...
	camera = std::make_shared<FPSCamera>(
		XMFLOAT3(0.0f, 5.0f, -15.0f),	// Position
		5.0f,					// Move speed
		0.002f,					// Look speed
	...
	// Set up draw data
	drawData = {};
	drawData.reflectionCount = 0;
	drawData.ambientAmount = 0.25f;
	drawData.shadows = 0;
	drawData.shadowSpread = 0.2f;
}
...
void Game::CreateRootSigAndPipelineState()
{
	...
	// Load shaders
	{
		...
		D3DReadFileToBlob(FixPath(L"SDF_CS.cso").c_str(), computeShaderByteCode.GetAddressOf());
	}
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	// Run compute shader
	{
		...
		XMStoreFloat4x4(&drawData.inverseViewProjection, invVP);
		...
		computeData.skyboxIndex = sky->GetSkyboxDescriptorIndex();
...
void Game::BuildUI()
{
	...
	ImGui::Begin("Inspector");
	{
		...
		{
			ImGui::SliderInt("Reflection Count", &drawData.reflectionCount, 0, 8);
			ImGui::SliderFloat("Ambient Amount", &drawData.ambientAmount, 0.0f, 1.0f);
			ImGui::Checkbox("Shadows", reinterpret_cast<bool*>(&drawData.shadows));

			if (drawData.shadows)
				ImGui::SliderFloat("Shadow Spread", &drawData.shadowSpread, 0.01f, 3.0f);
