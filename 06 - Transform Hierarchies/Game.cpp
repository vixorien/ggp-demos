...
void Game::CreateGeometry()
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		std::shared_ptr<GameEntity> current = entities[i];
		...
		for (int ch = 0; ch < 4; ch++)
		{
			...
			std::shared_ptr<GameEntity> child = std::make_shared<GameEntity>(sphereMesh);
	...
	{
		// Make the child, attached to the parent, moved slighty and scaled down
		std::shared_ptr<GameEntity> child = std::make_shared<GameEntity>(cubeMesh);
