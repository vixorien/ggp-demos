...
void Game::LoadAssetsAndCreateEntities()
{
	...
	// === Create the scene ===
	std::shared_ptr<GameEntity> shapesCube = std::make_shared<GameEntity>(assets.GetMesh(L"Meshes/cube"), parallaxShapesMat);
	//shapesCube->GetTransform()->SetScale(3);
	shapesCube->GetTransform()->SetPosition(0, 0, 0);
	entities.push_back(shapesCube);

	std::shared_ptr<GameEntity> leatherCube = std::make_shared<GameEntity>(assets.GetMesh(L"Meshes/cube"), parallaxLeatherMat);
	//leatherCube->GetTransform()->SetScale(3);
	leatherCube->GetTransform()->SetPosition(-5, 0, 0);
	entities.push_back(leatherCube);

	std::shared_ptr<GameEntity> bricksCube = std::make_shared<GameEntity>(assets.GetMesh(L"Meshes/cube"), parallaxBricksMat);
	//bricksCube->GetTransform()->SetScale(3);
	bricksCube->GetTransform()->SetPosition(5, 0, 0);
	entities.push_back(bricksCube);

	std::shared_ptr<GameEntity> plane = std::make_shared<GameEntity>(assets.GetMesh(L"Meshes/quad_double_sided"), parallaxStonesMat);
...
{
	Assets& assets = Assets::GetInstance();
	std::shared_ptr<Mesh> lightMesh = assets.GetMesh(L"Meshes/sphere");
