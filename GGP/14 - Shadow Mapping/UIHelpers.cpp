	...
	DemoShadowOptions& shadowOptions)
{
	...
	ImGui::Begin("Inspector");
	{
		...
		if (ImGui::TreeNode("Shadows")) 
		{
			...
			ImGui::Spacing();
			ImGui::Text("Shadow Map Resolution: %i", shadowOptions.ShadowMapResolution);
			if (ImGui::Button("Smaller"))
			{
				// Shrink the size, clamping at 64
				shadowOptions.ShadowMapResolution = max(64, shadowOptions.ShadowMapResolution / 2);
			}

			ImGui::SameLine();
			if (ImGui::Button("Larger"))
			{
				// Grow the size, clamping at 4096
				shadowOptions.ShadowMapResolution = min(4096, shadowOptions.ShadowMapResolution * 2);
			}

			ImGui::Image(shadowOptions.ShadowSRV.Get(), ImVec2(256, 256));
