...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	// SpriteFont: https://github.com/microsoft/DirectXTK/wiki/SpriteFont
	{
		...
		fontArial16->DrawString(spriteBatch.get(), "Only a single directional light is active to", XMFLOAT2(10, 10));
		fontArial16->DrawString(spriteBatch.get(), "ensure gamma correction results are obvious.", XMFLOAT2(10, 30));
		fontArial16->DrawString(spriteBatch.get(), "Ambient is also set to (0,0,0).", XMFLOAT2(10, 60));
			

		std::string text = "Press G to toggle gamma correction: ";
		XMVECTOR textSize = fontArial16->MeasureString(text.c_str(), false);
		fontArial16->DrawString(spriteBatch.get(), text.c_str(), XMFLOAT2(10, 90));

		XMVECTOR color = gammaCorrection ? XMVectorSet(0, 1, 0, 1) : XMVectorSet(1, 0, 0, 1);
		fontArial16Bold->DrawString(spriteBatch.get(), gammaCorrection ? "Enabled" : "Disabled", XMFLOAT2(XMVectorGetX(textSize), 90), color);
